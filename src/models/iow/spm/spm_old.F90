...
!BOP
!
! !MODULE: fabm_iow_spm_old --- 1-class SPM model,
!
! !INTERFACE:
   module fabm_iow_spm_old
!
! !USES:
   use fabm_types
...
! !PUBLIC DERIVED TYPES:
   type, extends(type_base_model),public :: type_iow_spm_old
...
! !INPUT PARAMETERS:
   ...
   class (type_iow_spm_old),           intent(inout), target :: self
...
!BOC
   ...
   ! Read the namelist
   if (configunit>0) read(configunit,nml=iow_spm_old,err=99,end=100)
...
99 call self%fatal_error('iow_spm_old_create','Error reading namelist iow_spm_old')

100 call self%fatal_error('iow_spm_old_create','Namelist iow_spm_old was not found')
...
! !INPUT PARAMETERS:
   class (type_iow_spm_old), intent(in) :: self
...
!BOP
!
! !ROUTINE: iow_spm_old_get_vertical_movement
!
! !INTERFACE:
...
!BOP
!
! !ROUTINE: iow_spm_old_get_light_extinction
!
! !INTERFACE:
