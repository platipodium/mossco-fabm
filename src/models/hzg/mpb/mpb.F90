...
!     Variable identifiers
      type (type_state_variable_id)        :: id_fdet, id_oxy, id_no3, id_nh4, id_tdic
...
!     Model parameters
      ...
      logical  :: use_fdet, use_oxy, use_no3, use_nh4, use_tdic
      real(rk) :: fdet_conc, oxy_conc, no3_conc, nh4_conc
...
!!------- Optional external dependencies of model mpb -------
   character(len=attribute_length) :: fdet_variable = ''  ! fast decaying detritus C
   ...
   character(len=attribute_length) :: tdic_variable = ''  ! total dissolved inorganic carbon
!    real(rk)  :: fdet_default_conc ! fast detritus C
!    real(rk)  :: oxy_default_conc  ! dissolved oxygen
!    real(rk)  :: no3_default_conc  ! dissolved nitrate
!    real(rk)  :: nh4_default_conc  ! dissolved ammonium
...
!!------- Parameters for model mpb originating from omexdia_p -------
   logical   :: MPhytoBenOn   ! use MicroPhytoBenthos (MPB)
   real(rk)  :: rFdet         ! decay rate fast decaying det.
   real(rk)  :: rSdet         ! decay rate slow decaying det.
   real(rk)  :: rNCfdet       ! NC ratio fast decaying det.
!!------- Parameters for model mpb -------
   ...
   namelist /hzg_mpb/ MPhytoBenOn, rFdet, rSdet, rNCfdet, &
   ...
   namelist /hzg_mpb_dependencies/  &
          fdet_variable, oxy_variable, no3_variable, nh4_variable, tdic_variable!, &
          !fdet_default_conc, oxy_default_conc, no3_default_conc, nh4_default_conc
!EOP
!-----------------------------------------------------------------------
!BOC
   ...
   !self%fdet_conc = fdet_default_conc
   ...
   self%use_fdet = fdet_variable/=''
   if (self%use_fdet) then
      call self%register_state_dependency(self%id_fdet, fdet_variable, 'mmolC/m**3',  &
            'fast detritus C', required=.true.)
      call self%request_coupling(self%id_fdet, fdet_variable)
   endif
   self%use_no3 = no3_variable/=''
   if (self%use_no3) then
      call self%register_state_dependency(self%id_no3, no3_variable, 'mmolN/m**3',   &
           'dissolved nitrate',        required=.false.)
   ...
   if (self%use_nh4) then
      call self%register_state_dependency(self%id_nh4, nh4_variable, 'mmolN/m**3',   &
            'dissolved ammonium',       required=.false.)
   ...
   self%use_tdic  = tdic_variable/=''
   if (self%use_tdic) then
     call self%register_state_dependency(self%id_tdic, tdic_variable, 'mmol/m**3',   &
           'total dissolved inorganic carbon', required=.false.)
      call self%request_coupling(self%id_tdic, tdic_variable)
   endif
...
! !LOCAL VARIABLES:
   ...
   real(rk) :: mpbC, mpbN, mpbCHL, eps, fdet, oxy, no3, nh4
...
!BOC
   ...
   if (self%use_fdet) _GET_(self%id_fdet, fdet) ! fast decaying detritus C in mmolC/m**3
   ...
   CprodEPS = sqrt(self%rFdet*self%rSdet) * eps
   ...
   faecesN    = self%kout * grazingC * self%rNCfdet ! Hochard et al 2010
...
#define _CONV_UNIT_ *one_pr_day
   ...
   if (self%use_fdet) _SET_ODE_(self%id_fdet, (faecesC) _CONV_UNIT_)
   ...
   if (_AVAILABLE_(self%id_tdic)) _SET_ODE_(self%id_tdic , (respzoo + respphyto) _CONV_UNIT_)
