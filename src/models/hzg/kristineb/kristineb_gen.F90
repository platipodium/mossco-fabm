...
real(rk),dimension(13)  :: Phy_initial,Phyto0,log_ESD  ! Phytoplankton biomass
real(rk)  :: N_initial    ! nutrient concentration 
real(rk)  :: P_initial    ! Phosphorous concentration 
real(rk),dimension(13)  :: Q_N_initial  ! Phytoplankton intracellular nitrogen cell quota 
...
real(rk),dimension(3)  :: sel          ! Selectivity, for each zooplankton add value to inside list
real(rk),dimension(3)  :: Lz           ! Zooplankton body size, log_e ESD, for each zooplankton add value to inside list	
real(rk)  :: I_max0       ! I_{max} at l_{z}=l_{z}^{*}=0
real(rk)  :: a_Im0        ! Size scaling exponent of I_{max}^{*}
real(rk),dimension(3)  :: Lz_star      ! Optimal prey size, log_e ESD, for each zooplankton add value to inside list
...
namelist /kristineb_pars/ &
  ...
  A_star_opt,R_A,tot_phyc0,log_ESD
...
call self%get_parameter(self%b_kn_N,	'b_kn_N',	default=b_kn_N)
call self%get_parameter(self%a_kn_N,	'a_kn_N',	default=a_kn_N)
...
call self%get_parameter(self%b_kn_P,	'b_kn_P',	default=b_kn_P)
call self%get_parameter(self%a_kn_P,	'a_kn_P',	default=a_kn_P)
...
!TODO: Why, explain!?
    do ib=1,phyto_num
        Phyto0(ib)=exp(-((log_ESD(ib)-2.1)**2)/(2*(0.45)**2))
    end do
    do ib=1,phyto_num
...
!Forcings
	...
	real(rk):: drat,par,SeaTemp,deepwTemp,pCO2 !TODO:?
...
!close(100)
    ...
    do cb=1,self%num_ciliat
      cop_pref(cb)=exp(-self%sel_cop_cil*(self%Lz_star(3)-self%Lz(cb))**2)
    end do
!----------------------
_LOOP_BEGIN_
...
!------------------------------------------   
 ...
 if (self%co2_low .eqv. .true.) then
   ...
   Zoo=(/cil_l/2.0_rk/12.0_rk,cil_l/2.0_rk/12.0_rk,4.0_rk*copepod_l*10.d-4/)
 else
   _GET_(self%id_cil_h,cil_h)
   _GET_(self%id_copepod_h,copepod_h)
   Zoo=(/cil_h/2.0_rk/12.0_rk,cil_h/2.0_rk/12.0_rk,4.0_rk*copepod_h*10.d-4/)
 end if
        !-- Temperature dependency
        call F_T(self,deepwTemp,SeaTemp,T_forcing)!F_T(t,self%tempdeep,self%tempsea,T_forcing)
        !-- pCo2 forcing
        call F_Co2sr(self,pCO2,f_co2)
        !-- PAR forcing
        ! PAR0 values
        call f_parsr(self,Phy,Q_N,f_co2,T_forcing,par,F_par)
!--------------------------
        ! Aggregation rate
	...
	do ib=1,self%phyto_num
        ...
        dD_N_dt_t = dD_N_dt(self, Phy, Q_N, D_N,aggr,T_forcing,grazing_forc)
        dD_P_dt_t = dD_P_dt(self, Phy, Q_P, D_P,aggr,T_forcing,grazing_forc)
