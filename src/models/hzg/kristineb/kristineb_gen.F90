...
module hzg_kristineb
   use fabm_types
   implicit none
   integer,parameter::numberofphytos=27
   private
!!----------------------------------------------------------------------
...
!TODO: Why, explain!?
    do ib=1,phyto_num
        Phyto0(ib)=1*exp(-((log_ESD(ib)-1.0)**2)/(2*(0.5)**2))+exp(-((log_ESD(ib)-4.)**2)/(2*(.5)**2))*0.1
    end do
!    do ib=10,phyto_num
!      Phyto0(ib)=0.1_rk
!   end do
...
!------------------------------------------   
 ...
 if (self%co2_low .eqv. .true.) then
   ...
   !Zoo=(/Phy(3)/2.0_rk,cil_l/2.0_rk/12.0_rk,4.0_rk*copepod_l*10.d-4/)
 else
...
!--------------------------
	...
	!dPhy_dt(3)=dPhy_dt(3)+(grazing_forc(1)+grazing_forc(2)+grazing_forc(3))/2.0*0.3
