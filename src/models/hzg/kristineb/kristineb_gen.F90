...
module hzg_kristineb
   use fabm_types
   implicit none
   integer,parameter::numberofphytos=27,numberofzoos=3
   private
!!----------------------------------------------------------------------
...
!Zooplankton Preferences
    ...
    do cb=1,self%num_ciliat
      cop_pref(cb)=exp(-self%sel_cop_cil*(self%Lz_star(numberofzoos-1)+self%Lz_star(numberofzoos)-self%Lz(cb))**2)
    end do
!----------------------
_LOOP_BEGIN_
...
!------------------------------------------   
 ...
 if (self%co2_low .eqv. .true.) then
   ...
   Zoo=(/cil_l/2.0_rk/12.0_rk,cil_l/2.0_rk/12.0_rk,csr*4.0_rk*copepod_l*10.d-4/)
   ...
   Zoo=(/cil_h/2.0_rk/12.0_rk,cil_h/2.0_rk/12.0_rk,csr*4.0_rk*copepod_h*10.d-4/)
 end if
