...
module hzg_kristineb
   use fabm_types
   implicit none
   integer,parameter::numberofphytos=13
   private
!!----------------------------------------------------------------------
...
!Scaling-Parameters
real(rk) :: b_mumax,a_mumax,b_mumax_large,a_mumax_large,b_mumax_small,a_mumax_small,mumax_incr,b_qmin_N,a_qmin_N,b_qmax_N,a_qmax_N,b_vmax_N,a_vmax_N,b_carbon,a_carbon
...
!TODO: Why, explain!?
    do ib=1,phyto_num
        Phyto0(ib)=exp(-((log_ESD(ib)-Phyto0_mean)**2)/(2*(Phyto0_sigma)**2))
    end do
 !   do ib=10,phyto_num
 !     Phyto0(ib)=0.1_rk
 !  end do
