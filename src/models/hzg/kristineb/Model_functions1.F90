...
!------------------------------------------------------------------------------
real(rk) function aggr_rate(self, Phy,Q_N,D_N,QNdot)! Aggregation rate, d^-1
implicit none
 class (type_hzg_kristineb),intent(in) :: self
 real(rk),dimension(self%phyto_num),intent(in) :: Phy! Phytoplankton biomass concentration, mmol-C m^-3
 real(rk),dimension(self%phyto_num),intent(in) :: Q_N! Phytoplankton intracellular nitrogen cell quota, mol-N mol-C^-1
 ...
 real(rk),intent(in)::QNdot
 real(rk)::TEP
  phy_conc=0.0_rk
	Do i=1,self%phyto_num
        	phy_conc(i)=Phy(i)*Q_N(i)
	end do
        TEP=(1._rk/(1._rk+exp(self%B_star_opt*QNdot+3)))
 !       Write(*,*)TEP
...
!         healthy, non-limited cells create buouyancy
 !           physiol = exp(-4.0_rk*qN*qP)
            physiol = exp(-0.5*((qP*qN)/0.2**2)**2) !Todo: Why?
!         size dependency: Stokes - vacuolation
       !     sinking(i)=physiol*exp(0.5_rk*self%log_ESD(i))* 0.06_rk/mixl
            sinking(i)=physiol*exp(0.5_rk*self%log_ESD(i))* 0.13_rk/mixl
...
implicit none
 ...
 grazing=0.0_rk
	if (self%graz_forc .eqv. .true.) then
	    ...
	    Do j=1,self%zoo_num
	      G=sum(graz_j(j,:))
	      downreg=1.0_rk/(1.0_rk+exp(-(G-G_min)/0.5_rk))
...
!------------------------------------------------------------------------------
subroutine Rel_growth_rate_sr(self,Phy,aggregation,growth_rate,grazing_forc,respiration,sinking,hetgraz,rel_growth_rate)
	implicit none
	class (type_hzg_kristineb),intent(in) :: self
 ...
 real(rk),dimension(self%phyto_num,2),intent(in) :: hetgraz !Heterotrophic grazing (prey,predators)
 integer::i
...
!Write(*,*)hetgraz(:,2)
        do i=1,self%phyto_num
            rel_growth_rate(i)=growth_rate(i)-respiration(i)-sinking(i)-self%m-aggregation-grazing_forc(i)/(eps+Phy(i)) + hetgraz(i,1) - hetgraz(i,2)
        end do
        return 
end subroutine
!------------------------------------------------------------------------------
subroutine hetero_graz(self,Phy,heterokernal,hetgraz)
implicit none
class (type_hzg_kristineb),intent(in) :: self
real(rk),dimension(self%phyto_num),intent(in)::Phy,heterokernal
real(rk),dimension(self%phyto_num,2),intent(out)::hetgraz
integer::i
hetgraz=0.0_rk
     do i=1,self%phyto_num
...
!------------------------------------------------------------------------------
real(rk) function chl_a(self,Phy,Q_N,par,mixl,chla_to_PhyN)
implicit none
 class (type_hzg_kristineb),intent(in) :: self
 real(rk),intent(in) :: par
 real(rk),dimension(self%phyto_num),intent(in) :: Phy! Phytoplankton biomass concentration, mmol-C m^-3
...
!        :return: Chl_a concentration
 integer::i
 real(rk) :: phyto_conc_tot,phyNtot,k,par_w,mixl
 real(rk),dimension(self%phyto_num)::phyN
 real(rk),intent(out)::chla_to_PhyN
...
!             par_w: Average light intensity within mixed layer depth,
	...
	chla_to_PhyN=self%chla_to_T_PhyN *self%I_opt/(self%I_opt+par_w)
        chl_a=phyto_conc_tot*chla_to_PhyN
        return
end function
!-----------------------------------------------------------------
