...
echo " b_affin_N2=10.0**b_affin_N" >> ${plotfile}
echo " b_affin_P2=10.0**b_affin_P" >> ${plotfile}
...
#
echo " b_affin_N_tmp=(b_affin_N2/b_carbon)*12.0*10.0**6  #moleX/moleC" >> ${plotfile}
echo " b_affin_N=b_affin_N_tmp*((pi/6.0)**(a_affin_N-a_carbon))" >> ${plotfile}
echo " a_affin_N=3.0*(a_affin_N-a_carbon)" >> ${plotfile}
#
echo " b_affin_P_tmp=(b_affin_P2/b_carbon)*12.0*10.0**6  #moleX/moleC" >> ${plotfile}
echo " b_affin_P=b_affin_P_tmp*((pi/6.0)**(a_affin_P-a_carbon))" >> ${plotfile}
echo " a_affin_P=3.0*(a_affin_P-a_carbon)" >> ${plotfile}
#----
...
echo "g(x)=b_Mumax_large2*exp(a_Mumax_large2*(x))" >> ${plotfile}
echo "set xlabel 'ln(ESD)'" >> ${plotfile}
echo "plot f(x) < g(x) ? f(x) : g(x) t sprintf(\"{/Symbol a}_{s}= %1.2f , {/Symbol b}_{s}= %1.2f  \n {/Symbol a}_{l}= %1.2f , {/Symbol b}_{l}= %1.2f \", a_Mumax_small2, b_Mumax_small2, a_Mumax_large2, b_Mumax_large2)" >> ${plotfile}
...
echo "set title 'affinity'" >> ${plotfile}
echo "f(x)=b_affin_N*exp(a_affin_N*x)" >> ${plotfile}
echo "g(x)=b_affin_P*exp(a_affin_P*x)" >> ${plotfile}
echo "plot f(x) t sprintf(\"N ({/Symbol a}= %1.2f , {/Symbol b}= %1.2f  )\", a_affin_N, b_affin_N), g(x) t sprintf(\"P ({/Symbol a}= %1.2f , {/Symbol b}= %1.2f  )\", a_affin_P, b_affin_P)" >> ${plotfile}
