...
if (self%IronOn) then
      _GET_(self%id_nutFe, nut%Fe)  ! Dissolved Inorganic Iron Fe in mmol-Fe/m**3
      _GET_(self%id_phyFe, phy%Fe)  ! Phytplankton Iron in mmol-Fe/m**3
      _GET_(self%id_detFe, det%Fe)  ! Detritus Iron in mmol-Fe/m**3
end if
...
! calls grazing function accounting for homeostasis
  ...
  nquot       = type_maecs_om(1.0_rk, phy%Q%N, phy%Q%P,  phy%Q%Si, phy%Q%Fe )

  !mswitch     = type_maecs_switch(self%PhosphorusOn,self%SiliconOn,.true. )
  mswitch     = type_maecs_switch(self%PhosphorusOn,self%SiliconOn,self%IronOn,.true. )
...
!  --- transform from specific to bulk grazing rate
  graz_rate   = graz_rate * zoo%C 
                                  ...
                                  !isP, isSi, isTotIng,isFe

else
  graz_rate   = 0.0_rk
!  if (self%ChemostatOn .and. .not. IsCritical) graz_rate = 0.2*phy%C
  zoo_mort    = 0.0_rk
  lossZ       = type_maecs_om(0.0_rk, 0.0_rk, 0.0_rk, 0.0_rk, 0.0_rk)
  floppZ      = type_maecs_om(0.0_rk, 0.0_rk, 0.0_rk, 0.0_rk,  0.0_rk)
end if
...
!   write (*,'(A,4(F10.3))') 'rhs DIP=',nut%P,self%nutP_initial,rhsv%nutP,Pprod
...
if (self%IronOn) then 
  ! ---  PHYTOPLANKTON Fe
   rhsv%phyFe = uptake%Fe              * phy%C    & 
              - self%dil             * phy%Fe    &            
              - aggreg_rate          * phy%Fe    & 
              - graz_rate            * phy%Q%Fe    
  !  --- DETRITUS Fe
   rhsv%detFe = floppZ%Fe              * zoo%C    &
              + aggreg_rate          * phy%Fe    &
              - self%dil             * det%Fe    &         
              - degradT              * det%Fe
  !  --- Dissolved Fe
   rhsv%nutFe = - uptake%Fe            * phy%C    & 
              + degradT              * det%Fe    & 
              + lossZ%Fe              * zoo%C    &
              + self%dil * (self%nutFe_initial - nut%Fe)
...
if (self%IronOn) then
      _SET_ODE_(self%id_nutFe, rhsv%nutFe UNIT)
      _SET_ODE_(self%id_phyFe, rhsv%phyFe UNIT)
      _SET_ODE_(self%id_detFe, rhsv%detFe UNIT)
end if   
if (self%GrazingOn) then
...
if (self%PhysiolDiagOn) then
  ...
  _SET_DIAGNOSTIC_(self%id_QFe, _REPLNAN_(phy%Q%Fe))         !average Fe:C_ratio_
  ...
  _SET_DIAGNOSTIC_(self%id_aVFe, _REPLNAN_(acclim%aV%Fe))    !average Fe-uptake_activity_
  ...
  _SET_DIAGNOSTIC_(self%id_faFe, _REPLNAN_(acclim%fA%Fe))    !average FE-uptake_affinity_allocation_
  ...
  _SET_DIAGNOSTIC_(self%id_rQFe, _REPLNAN_(phy%relQ%Fe))     !average Relative_Fe-Quota_
end if
...
!    _GET_(self%id_par, par)  ! light photosynthetically active radiation
...
!   endif
   ...
   if (self%IronOn) then
      _SET_VERTICAL_MOVEMENT_(self%id_phyFe,vs_phy)
      _SET_VERTICAL_MOVEMENT_(self%id_detFe,vs_det)
   end if
   if (self%PhotoacclimOn) then 
...
subroutine maecs_do_surface(self,_ARGUMENTS_DO_SURFACE_)
   ...
   real(rk) :: tot_vi_C,tot_vi_N,tot_vi_P,tot_vi_S,tot_vi_Fe, O2flux,O2airbl,oxy,tot_vi_GPPR,tot_vi_Denitr
...
write(*,'(A)') 'begin surface_DO'
      ...
      if (self%IronOn) then
         _GET_HORIZONTAL_(self%id_totFe_vertint,tot_vi_Fe)
         _SET_HORIZONTAL_DIAGNOSTIC_(self%id_totFe_vertint_diag,_REPLNAN_(tot_vi_Fe))
      end if
      end if
