...
!>   - phy\%Q\%X = phy\%X / phy\%C where x=N,P,Si,Fe
!>   - phy\%relQ\%X= (phy\%Q\%X - maecs\%qN_phy_0) / maecs\%iK_QN where
!x=N,P,Si,Fe
...
if (maecs%IronOn) then 
   phy%Q%Fe    = phy%Fe / phy%reg%C
! added for mixing effects in estuaries kw Jul, 15 2013
   phy%Q%Fe     = smooth_small(phy%Q%Fe, maecs%QFe_phy_0 + maecs%small_finite *  maecs%QFe_phy_max)

   phy%relQ%Fe = ( phy%Q%Fe - maecs%QFe_phy_0 ) /(maecs%QFe_phy_max-maecs%QFe_phy_0) 

!TODO extend max-reQ to Fe

! added for deep detritus traps with extreme quotas kw Jul, 16 2013
!   phy%relQ%Fe = _ONE_ - smooth_small(_ONE_- phy%relQ%Fe, maecs%small_finite)
!   phy%Q%FeN    = phy%Fei / phy%reg%N
end if 
...
!> - sens\%upt\_pot\%X, (@f$=V_X@f$), X=N,P,Si,Fe calculated by uptflex() \lref{see eq. ,eq:uptakecoeffcurr,.}
...
!Fe-uptake coefficients
if (maecs%IronOn) then 
   NutF          = smooth_small(nut%Fe,maecs%small)
! optimal partitioning 
   fA%Fe         = fOptUpt(maecs%AffFe,maecs%V_FeC_max * sens%f_T2, NutF, IsAdap)
   acc%fA%Fe     = fA%Fe
   sens%upt_pot%Fe= uptflex(maecs%AffFe,maecs%V_FeC_max * sens%f_T2,NutF,fA%Fe)
else
   acc%fA%Fe     = 0.5d0
   acc%Av%Fe     = 0.d0
end if
