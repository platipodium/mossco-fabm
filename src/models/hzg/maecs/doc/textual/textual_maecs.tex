...
% % --------------------------------------------------------------------------------------------- 
% -----------------------------------------------------------------------------------------
\begin{subsection}{Model structure}
...
The  correction coefficient $c_h$ in \Eq{eq:totc} follows from imposing convergence of $\climf$ to the product and Liebig rules. Compliance to the Liebig rule leads to $c_0=0$ since $g_0$ describes a stepwise linear function. For the product rule, we use the identity $g_h(1)=1/(1+h)$ and assume that $q_\nind=q_\mathrm{Y}=1/2$,  to obtain $1/4 = 1/2\cdot(1+h/4+c_h)/(1+h)$, or $c_h=-1/2+h/4$. However, the offset in this linear relation conflicts with the condition $c_0=0$. Both conditions can be approximately reconciled by a logarithmic function 
\beql{eq:ch}
c_h  = \log(1/4^h + h/2)
\eeq
